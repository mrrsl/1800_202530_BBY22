...
/**
 * Generates documents with default fields for the authenticated user.
 */
export const defaultEntry = function() {
    let prefRef = doc(firebaseDb, PREF_COLLECTION_NAME, firebaseAuth.currentUser.uid);
    let personalRef = doc(firebaseDb, PERSONAL_COLLECTION_NAME, firebaseAuth.currentUser.uid);

    getDoc(prefRef).then(snap => {
        if (!snap.exists()) {
            setDoc(prefRef, DEFAULT_PREFERENCES);
        }
    });
